...
public class MovieTest {
	...
	public void testIsReleaseDayFalseOneYearBeforeNow() throws Exception {
		boolean isRelease = getDefaultMovieBuilder()
				.withReleaseDate(DateTime.now().minusYears(1)).build()
				.isRelease();
	...
	public void testIsReleaseDayTrueWithNowMinusMovieClassReleaseConstantMinusOne()
		...
		boolean isRelease = getDefaultMovieBuilder()
				.withReleaseDate(
						DateTime.now().minusDays(Movie.DAYS_AS_RELEASE - 1))
				.build().isRelease();
		assertEquals(isRelease, true);
	}

	@Test
	public void testIsReleaseDayTrueWithNowMinusMovieClassReleaseConstantPlusOne()
			throws Exception {
		boolean isRelease = getDefaultMovieBuilder()
				.withReleaseDate(
						DateTime.now().minusDays(Movie.DAYS_AS_RELEASE + 1))
	...
	public void testMovieIsNotCommentableIfReleaseIsAfterNow() throws Exception {
		Movie movie = getDefaultMovieBuilder().withReleaseDate(
				DateTime.now().plusDays(2)).build();
	...
	private MovieBuilder getDefaultMovieBuilder() {
		...
		return Movie.builder().withDirector(director).withGenres(genres)
				.withReleaseDate(releaseDate).withSummary(summary)
				.withTitle(title).withRuntimeInMins(runtimeInMins);
	}
